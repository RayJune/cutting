 ...
 *
 * Time Complexity: O(n ** 2) = for 循环次数 O(n) + while 循环次数 O(n) * splice 操作 O(n)
 * Space complexity: O(n) = candidates 长度 O(n) + permutation 长度 O(n)
 * Auxiliary complexity: O(n) = candidates 长度 O(n)
 *
 * @param {number} n
 * @param {number} k
 * @returns {string}
...
function getPermutation(n, k) {
    ...
    while (candidates.length) {
        factorial /= candidates.length;
        permutation.push(candidates.splice(Math.floor(k / factorial), 1)[0]);
        k %= factorial;
    }
